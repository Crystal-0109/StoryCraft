...
async function expandText() {
    ...
    try {
        ...
        if (q && expanded) {
            ...
            if (
                snap &&
                Number.isFinite(snap.index) &&
                snap.length > 0 &&
                !window.__forceFullOnce
            ) {
                q.deleteText(snap.index, snap.length, 'user');
                q.insertText(snap.index, expanded, 'user');
                // 커서는 삽입 끝으로
                try {
                    q.setSelection(snap.index + expanded.length, 0, 'silent');
                } catch {}
            } else {
                q.setText(expanded);
                try {
                    q.setSelection(Math.max(0, q.getLength() - 1), 0, 'silent');
                } catch {}
            }
                        ...
                        if (mode === 'length') {
                            ...
                            if (btnApply) {
                                ...
                                btnApply.onclick = () => {
                                    ...
                                    const hasLiveSel = !!(
                                        live && typeof live.index === 'number'
                                    );
                                    const hasRange =
                                        hasLiveSel && live.length > 0;
